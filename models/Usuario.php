...
class Usuario extends Conectar
{
    ...
    public function login()
    {
        ...
        if (isset($_POST["enviar"])) {
            ...
            if (empty($correo) and empty($pass)) {
                header("Location:" . conectar::ruta() . "indexLoginMant.php?m=2");
                exit();
            } else {
                ...
                if (is_array($resultado) and count($resultado) > 0) {
                    ...
                    header("Location:" . Conectar::ruta() . "indexLoginMant.php?m=1");
                    exit();
                }
            }
        }
    ...
    /* TODO: Obtener registros de usuarios segun rol tecnico */
    public function get_usuario_x_rol_tecnico()
    {
        $conectar = parent::conexion();
        parent::set_names();
        $sql = "SELECT * FROM tm_usuario where est=1 and rol_id=3";
    ...
    /* TODO: Total de registros segun usu_id */
    public function get_usuario_total_x_id_responsable($usu_id)
    {
        $conectar = parent::conexion();
        parent::set_names();
        $sql = "SELECT COUNT(*) as TOTAL FROM tm_ticket where tm_ticket.usu_asig = ?";
        $sql = $conectar->prepare($sql);
        $sql->bindValue(1, $usu_id);
        $sql->execute();
        return $resultado = $sql->fetchAll();
    ...
    public function get_usuario_totalabierto_x_id($usu_id)
    {
        ...
        where usu_id = ? 
        AND (tick_estado = 'Cliente' 
        OR tick_estado = 'Asignado'
        OR tick_estado = 'Asignado Con Materiales' 
        OR tick_estado = 'En Compras' 
        OR tick_estado = 'Pendiente Materiales' 
        ...
        /* TODO: Total de Tickets Abiertos por usu_id */
        public function get_usuario_total_x_visto_bueno($usu_id)
        {
            $conectar = parent::conexion();
            parent::set_names();
            $sql = "SELECT COUNT(*) as TOTAL FROM tm_ticket 
            where usu_id = ? 
            AND tick_estado = 'Cierre Técnico'";
            $sql = $conectar->prepare($sql);
            $sql->bindValue(1, $usu_id);
            $sql->execute();
            return $resultado = $sql->fetchAll();
