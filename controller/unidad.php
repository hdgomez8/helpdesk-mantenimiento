...
switch ($_GET["op"]) {
    ...
    case "nombre_unidad";
        $datos = $unidad->get_nombre_unidad($_POST["unidad"]);
        if (is_array($datos) == true and count($datos) > 0) {
            foreach ($datos as $row) {
                $output["unidad_id"] = $row["unidad_id"];
                $output["unidad_nombre"] = $row["unidad_nombre"];
            }
            echo json_encode($output);
        }
        break;
    ...
    case "combo":
        ...
        $html = "";
        $html .= "<option value='Seleccionar' disabled selected>Seleccionar</option>";
        if (is_array($datos) == true and count($datos) > 0) {
            foreach ($datos as $row) {
                $html .= "<option value='" . $row['unidad_id'] . "'>" . $row['unidad_nombre'] . "</option>";
            }
            echo $html;
        }
        break;
